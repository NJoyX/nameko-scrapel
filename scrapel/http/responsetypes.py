...
class ResponseTypes(object):
    ...
    def __init__(self):
        ...
        mimedata = get_data('scrapel', 'mime.types').decode('utf8')
        self.mimetypes.readfp(StringIO(mimedata))
        for mimetype, cls in six.iteritems(self.CLASSES):
            self.classes[mimetype] = cls
    ...
    def register_mimetype(self, mimetype, response_cls):
        assert issubclass(response_cls, Response), 'Incorrect response class for "{}" mimetype!'.format(mimetype)
        self.CLASSES[mimetype] = response_cls
