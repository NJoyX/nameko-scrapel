    ...
    def __init__(self, *args, **kwargs):
        ...
        if formdata:
            if self.method == 'POST':
                ...
                querystr = self.querystr(formdata)
                self._set_url(self.url + ('&' if '?' in self.url else '?') + querystr)

    def querystr(self, formdata):
        items = formdata.items() if isinstance(formdata, dict) else formdata
        return _urlencode(items, self.encoding)

    def _set_body(self, body):
        if isinstance(body, dict):
            self._body = body
        else:
            querystr = self.querystr(body)
            super(FormRequest, self)._set_body(body=querystr)
